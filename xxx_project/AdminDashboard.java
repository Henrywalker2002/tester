...
//    private TableView tableView;
    ...
    void onSignOutClick(ActionEvent event) throws IOException {
        BorderPane pane = FXMLLoader.load(getClass().getResource("f_files/Login_screen.fxml"));
    ...
    public void onAddFavorite(ActionEvent event) throws Exception {
        ...
        String item_name = item_name_field.getText();
        Integer USERID = database.session;
        ...
        boolean favorite_exists = database.favoriteItemExists(id_item, USERID);
        ...
        if(favorite_exists)
        {
            ...
            database.addFavorite(id_item, item_name, USERID);
            showFavorites();
            item_name_field.clear();
            item_field.clear();
        }
